...
export const sendToAgentAction: Action = {
  ...
  parameters: [
    { name: "sessionId", description: "Target task-agent session ID", required: false, schema: { type: "string" } },
    { name: "input", description: "Text to send to the agent", required: false, schema: { type: "string" } },
    { name: "task", description: "New task to assign to the agent", required: false, schema: { type: "string" } },
    { name: "label", description: "Optional task label", required: false, schema: { type: "string" } },
    { name: "keys", description: "Key sequence to send", required: false, schema: { type: "string" } },
  ],
  ...
  handler: async (runtime, message, state, options, callback): Promise<ActionResult> => {
    ...
    try {
      ...
      if (!target.session) {
        ...
        await callbackText(callback, "No active task-agent sessions. Spawn an agent first.");
      ...
      if (keys) {
        ...
        return { success: true, text: "Sent key sequence", data: { sessionId: target.session.id, keys } };
      }
      ...
      if (textInput) {
        ...
        return {
          success: true,
          text,
          data: { sessionId: target.session.id, input: textInput, ...(task ? { task } : {}) },
        };
      }

      await callbackText(callback, "No input provided. Specify 'input', 'task', or 'keys' parameter.");
