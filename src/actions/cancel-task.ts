...
export const cancelTaskAction: Action = {
  name: "CANCEL_TASK",
  similes: ["STOP_TASK", "CANCEL_AGENT_TASK", "CANCEL_TASK_AGENT", "ABORT_TASK", "KILL_TASK", "STOP_SUBTASK"],
  description:
  ...
  parameters: [
    { name: "threadId", description: "Task thread ID", required: false, schema: { type: "string" } },
    { name: "sessionId", description: "Session ID", required: false, schema: { type: "string" } },
    { name: "search", description: "Search text for a matching task", required: false, schema: { type: "string" } },
    { name: "all", description: "Cancel all active tasks", required: false, schema: { type: "boolean" } },
    { name: "reason", description: "Cancellation reason", required: false, schema: { type: "string" } },
  ],
  ...
  handler: async (runtime, message, state, options, callback): Promise<ActionResult> => {
    ...
    try {
      ...
      const sessionId = pickString(params, content, "sessionId") ?? (state as unknown as { codingSession?: { id?: string } } | undefined)?.codingSession?.id;
      ...
      if (all) {
        ...
        for (const session of sessions) {
          await (service.cancelSession?.(session.id) ?? service.stopSession(session.id));
          stoppedSessions.push(session.id);
        }
        const text = `Canceled ${stoppedSessions.length} task(s).`;
        await callbackText(callback, text);
        return { success: true, text, data: { canceledCount: stoppedSessions.length, stoppedSessions } };
      }

      const target = sessionId
        ? await Promise.resolve(service.getSession(sessionId))
        : search
          ? sessions.find((session) => `${session.id} ${session.name ?? ""} ${session.metadata?.label ?? ""}`.toLowerCase().includes(search))
          : newestSession(sessions);

      if (!target) {
        const code = sessionId ? "SESSION_NOT_FOUND" : "TASK_NOT_FOUND";
        const text = sessionId ? `Session ${sessionId} not found.` : "No matching task found.";
        await callbackText(callback, text);
        return errorResult(code);
      }

      await (service.cancelSession?.(target.id) ?? service.stopSession(target.id));
