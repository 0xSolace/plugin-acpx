...
export const listAgentsAction: Action = {
  ...
  handler: async (runtime, _message, _state, _options, callback): Promise<ActionResult> => {
    ...
    if (sessions.length === 0) {
      const text = "No active task agents. Use CREATE_TASK when the user needs anything more involved than a simple direct reply.";
      await callbackText(callback, text);
      return { success: true, text, data: { sessions: [], tasks, pendingConfirmations, preferredTaskAgent } };
    }

    const lines = [`Active task agents (${sessions.length}):`];
    for (const session of sessions) {
      lines.push(`- ${labelFor(session)} [${shortId(session.id)}] ${session.agentType} ${session.status} in ${session.workdir}`);
    }
