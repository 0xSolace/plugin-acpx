...
describe("InMemorySessionStore", () => {
  ...
  it("updates lastActivityAt on status transitions", async () => {
    ...
    expect(blocked?.lastActivityAt.getTime()).toBeGreaterThan(
      new Date("2026-05-03T10:00:00.000Z").getTime(),
    );
    ...
    expect(errored?.lastActivityAt.getTime()).toBeGreaterThanOrEqual(
      blocked?.lastActivityAt.getTime() ?? 0,
    );
