...
Canonical reference for the `acpx` CLI as consumed by `@miladyai/plugin-acpx`. Authoritative source: `.research/acpx-docs/` (ocplatform/acpx README, docs/CLI.md, skills/acpx/SKILL.md, ACP coverage roadmap). All citations refer to those files.
...
### What `@miladyai/plugin-acpx` uses
...
Out of scope for `@miladyai/plugin-acpx` v0.1.0. Mentioned for completeness.
...
For `@miladyai/plugin-acpx`, the plugin runs in a server context with no TTY. Strategy:
...
- Target: `acpx@latest` at time of plugin v0.1.0-rc.1 release.
