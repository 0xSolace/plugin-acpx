...
Canonical reference for the `acpx` CLI as consumed by `@elizaos/plugin-acp`. Authoritative source: `.research/acpx-docs/` (ocplatform/acpx README, docs/CLI.md, skills/acpx/SKILL.md, ACP coverage roadmap). All citations refer to those files.
...
Out of scope for `@elizaos/plugin-acp` v0.1.0. Mentioned for completeness.
...
For `@elizaos/plugin-acp`, the plugin runs in a server context with no TTY. Strategy:
